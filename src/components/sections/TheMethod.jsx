...
export const TheMethod = ({ onNavigate }) => {
    ...
    return (
        <SectionWrapper id="method" className="!pt-20 !pb-10">
            <div className="grid md:grid-cols-2 gap-4 md:gap-16 items-start h-full md:h-auto overflow-y-auto md:overflow-visible pb-12 md:pb-0">
                <div className="order-1 md:order-1 md:sticky top-0 md:top-24 bg-slate-50 md:bg-transparent z-10 py-2 md:py-0">
                    <Reveal><SectionTitle subtitle="Filosofía VDTM" title="El Método Híbrido" /></Reveal>
                    <Reveal delay={100}><p className="text-slate-600 mb-3 md:mb-6 leading-relaxed text-sm md:text-lg">Dejamos atrás los modelos rígidos. En VDTM, nuestra filosofía se basa en la fluidez entre la potencia computacional y la sensibilidad humana.</p></Reveal>
                    <Reveal delay={200}>
                        <div className="bg-blue-50 p-3 md:p-6 rounded-xl border border-blue-100 hover:shadow-md transition-shadow duration-300">
                            <h4 className="font-bold text-blue-900 mb-1 flex items-center gap-2 text-sm md:text-base">
                                <CheckCircle2 className="w-4 h-4 text-blue-600 animate-pulse" /> ¿Por qué funciona?
                            </h4>
                            <p className="text-blue-800/80 text-[10px] md:text-sm">Elimina tareas repetitivas, permitiendo enfocarse al 100% en la estrategia.</p>
                        </div>
                    </Reveal>
                </div>
                        ...
                        <Reveal key={idx} delay={idx * 150}>
                            <div className={`rounded-xl border transition-all duration-300 overflow-hidden cursor-pointer group ${openIndex === idx ? 'bg-white border-blue-200 shadow-lg' : 'bg-slate-50 border-slate-200 hover:bg-white'}`} onClick={() => setOpenIndex(idx === openIndex ? -1 : idx)}>
                                ...
                                <div className={`transition-all duration-500 ease-in-out overflow-hidden ${openIndex === idx ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="px-3 md:px-6 pb-3 md:pb-6 pl-14 md:pl-20"><p className="text-slate-600 leading-relaxed text-xs md:text-sm">{item.content}</p></div>
                                </div>
                            </div>
                        </Reveal>
                    ))}
